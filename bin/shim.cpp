...
std::tuple<std::wstring_p, std::wstring_p, std::wstring_p> GetShimInfo()
{
    ...
    while (true)
    {
        ...
        if (line.substr(0, 3) == L"dir" && line.size() > 7)
        {
            dir.emplace(line.data() + 6, line.size() - 6 - (line.back() == L'\n' ? 1 : 0));
            continue;
        }
    }
...
std::tuple<std::unique_handle, std::unique_handle> MakeProcess(const std::wstring_p& path, const std::wstring_p& args, const std::wstring_p& dir)
{
    // Start subprocess
    STARTUPINFOW si = {};
    ...
    if (CreateProcessW(nullptr, cmd.data(), nullptr, nullptr, TRUE, CREATE_SUSPENDED, nullptr, dir.value().empty() ? nullptr : dir.value().c_str(), &si, &pi))
    {
...
{
    auto [path, args, dir] = GetShimInfo();
    ...
    // if base dir not exists, pass null
    if (!dir || !(fs::is_directory(fs::path{ dir.value() }) && fs::exists(fs::path{ dir.value() }))) {
        dir.emplace();
    }
    ...
    auto [processHandle, threadHandle] = MakeProcess(std::move(path), std::move(args), std::move(dir));
